...
REF_IMGS_DIR = osp.join("1obj_renders_large")  
OUTPUT_DIR = f"1obj_canny_{CONDITIONING_SCALE}_{NUM_INFERENCE_STEPS}/" 
DEBUG_DIR = f"1obj_canny_{CONDITIONING_SCALE}_{NUM_INFERENCE_STEPS}__debug" 
BS = 6  
USE_VIEW_HINTS = True 
...
if __name__ == "__main__": 
    ...
    prompts_file = "prompts/prompts_3008.txt" 
    ...
    # for subject_ in [sys.argv[-1].strip()]:   
        ...
        subject1 = " ".join(subject_pair_.split("_")) 
        ...
        with accelerator.split_between_processes(list(range(len(template_prompts))), apply_padding=False) as prompt_ids: 
            ...
            for prompt_idx in prompt_ids: 
                ...
                for start_idx in range(0, len(img_paths), BS):  
                    ...
                    for idx in range(end_idx - start_idx):  
                        assert "PLACEHOLDER" in template_prompt, f"{template_prompt = }" 
                        if not USE_VIEW_HINTS: 
                            prompt = template_prompt.replace(f"PLACEHOLDER", f"a {subject1}")  
                            prompts.append(prompt.strip()) 
                            continue 

                        img_name = img_names[start_idx + idx] 
                        subject1_coords, _ = img_name.split("__") 
                        ...
                        prompt = template_prompt.replace(f"PLACEHOLDER", "a SUBJECT1").strip()  
