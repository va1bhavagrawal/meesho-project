...
direction = mathutils.Vector([-1, 0, -0.15]) 
...
for i in range(len(obj_paths)):  
    for j in range(i, len(obj_paths)):  
        ...
        while successful_renders < NUM_RENDERS: 
            # obj1_loc = get_random_location() 
            # obj2_loc = get_random_location() 
            obj2_loc = (VIEW_FRUSTUM["near"][0], VIEW_FRUSTUM["near"][1], 0)  
            obj1_loc = (VIEW_FRUSTUM["far"][0], VIEW_FRUSTUM["far"][1], 0)  


            # print(f"{obj1_loc = }")
            # print(f"{obj2_loc = }")

            obj1_azimuth = math.radians(45)  
            obj2_azimuth = math.radians(0)   
            # obj1_azimuth = 90  
            # obj2_azimuth = 90 
            ...
            save_path = osp.join(OUTPUT_DIR, dict_key) 
