...
if __name__ == "__main__": 
    ...
    REF_IMGS_DIR = "a_shifted_blackbg_scaled/"  
    OUTPUT_DIR = f"a_shifted_scaled_cannysd_{CONDITIONING_SCALE}_{NUM_INFERENCE_STEPS}steps" 
    BS = 60  
    ...
    for subject_ in os.listdir(REF_IMGS_DIR):  
    # for subject_ in [sys.argv[-1].strip()]:   
        subject = " ".join(subject_.split("_")) 
        prompt_ids = range(10, 14, 1)    
        ...
        for prompt_idx in prompt_ids: 
            ...
            img_names = sorted([img_name for img_name in img_names if img_name.find(f".jpg") != -1])  
            img_names = random.sample(img_names, 360)   
            ...
            for start_idx in range(0, len(img_paths), BS):  
                ...
                for idx in range(end_idx - start_idx):  
                    ...
                    prompts.append(prompt.replace("a SUBJECT", f"a photo of a {subject}").strip())  
                    print(f"{prompts[-1]}")
                ...
                negative_prompt = "bad quality, worst quality, sketches, unrealistic"
                output_imgs = pipe(prompts, image=canny_imgs, negative_prompt=[negative_prompt] * len(canny_imgs), num_inference_steps=NUM_INFERENCE_STEPS, controlnet_conditioning_scale=CONDITIONING_SCALE).images   
