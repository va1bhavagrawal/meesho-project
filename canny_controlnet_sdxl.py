...
if __name__ == "__main__": 
    ...
    OUTPUT_DIR = f"a_shifted_scaled_cannyxl_{CONDITIONING_SCALE}/" 
    BS = 18    
    ...
    # for subject_ in [sys.argv[-1].strip()]:   
        ...
        for prompt_idx in prompt_ids: 
            ...
            img_names = random.sample(img_names, 18)   
            ...
            for start_idx in range(0, len(img_paths), BS):  
                ...
                print(f"saving the canny images...")
                for canny_idx, canny_img in enumerate(canny_imgs): 
                    canny_img.save(f"{str(canny_idx).zfill(3)}.jpg") 
                # print(f"{canny_imgs = }")
                for canny_img in canny_imgs: 
                    assert canny_img is not None 
                # image = image[:, :, None]
                # image = np.concatenate([image, image, image], axis=2)
                # image = Image.fromarray(image)
                print(f"{len(prompts) = }")
                print(f"{len(canny_imgs) = }")
