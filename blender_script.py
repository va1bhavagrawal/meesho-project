...
def render_object(
    ...
    for i in range(num_renders):
        ...
        radius = radius_low + radius * (radius_high - radius_low) / 100 

        elevation_low = 0.0 
        elevation_high = math.pi / 2 
        elevation = np.random.randint(0, 100) 
        elevation = elevation_low + elevation * (elevation_high - elevation_low) / 100 

        azimuth_low = 0.0 
        azimuth_high = 2 * math.pi 
        azimuth = np.random.randint(0, 100) 
        azimuth = azimuth_low + azimuth * (azimuth_high - azimuth_low) / 100 

        print(f"{radius = }, {elevation = }, {azimuth = }")
        
        # x = radius * math.cos(i * 2 * math.pi / num_renders) 
        # y = radius * math.sin(i * 2 * math.pi / num_renders) 
        # z = 0.0

        z = math.sin(elevation) * radius 
        x = (math.cos(elevation) * radius) * math.cos(azimuth) 
        y = (math.cos(elevation) * radius) * math.sin(azimuth) 

        assert round(x * x + y * y + z * z, 2) == round(radius * radius, 2), f"{x * x + y * y + z * z = }, {radius * radius = }" 
        camera.location = Vector(np.array([x, y, z]))
        direction = -camera.location
        rot_quat = direction.to_track_quat("-Z", "Y")
        camera.rotation_euler = rot_quat.to_euler()
        ...
        np.save(os.path.join(output_dir, f"xyz_{i:03d}.npy"), [*camera.location])
        aer = {
        ...
        print(f"{aer = }")
        save_path = os.path.join(output_dir, f"aer_{i:03d}.pkl") 
        print(f"{save_path = }")
        with open(save_path, "wb") as f:  
            pickle.dump(aer, f) 
            # pickle.dump(f, aer) 
            # pass 
        # np.save(os.path.join(output_dir, f"aer_{i:03d}.npy"), {
        #     "azimuth": azimuth, 
        #     "elevation": elevation, 
        #     "radius": radius, 
        # })
...
if __name__ == "__main__":
    ...
    # Render the images
    print(f"doing {args.object_path}") 
    render_object(
