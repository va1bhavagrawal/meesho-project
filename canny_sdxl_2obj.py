...
if __name__ == "__main__": 
    ...
    REF_IMGS_DIR = "pickup_truck__motorbike/"  
    OUTPUT_DIR = f"canny_pickup_truck__motorbike_{CONDITIONING_SCALE}/" 
    BS = 6     

    prompts_file = "prompts/prompts_2208.txt" 
    with open(prompts_file, "r") as f: 
        all_prompts = f.readlines() 
        all_prompts = [str(prompt).strip() for prompt in all_prompts] 
    ...
    # for subject_ in [sys.argv[-1].strip()]:   
        ...
        for prompt_idx in prompt_ids: 
            ...
            subject_path = osp.join(REF_IMGS_DIR)  
            ...
            for start_idx in range(0, len(img_paths), BS):  
                ...
                for idx in range(end_idx - start_idx):  
                    ...
                    prompts.append(prompt.replace("a SUBJECT", f"a realistic photo of a pickup truck and a motorbike").strip())  
                    print(f"{prompts[-1]}")
                ...
                for idx, img in enumerate(output_imgs):  
                    ...
                    save_path = osp.join(save_path, f"{inp_img_name.split(f'.jpg')[0]}__prompt{prompt_idx}.jpg")  
                    img.save(save_path) 
