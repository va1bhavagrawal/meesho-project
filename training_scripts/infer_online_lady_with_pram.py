...
class Infer: 
    ...
    def generate_images_in_a_batch_and_save_them(self, batch, step_idx): 
        ...
        for batch_idx in range(len(all_assignments)): 
            ...
            for asset_idx in range(len(all_assignments[batch_idx])): 
                if asset_idx == 0: 
                    bboxes_example.append(torch.tensor([0.25, 0.40, 0.90, 1.00]))   
                elif asset_idx == 1: 
                    bboxes_example.append(torch.tensor([0.525, 0.20, 1.00, 1.00]))    
                elif asset_idx == 2: 
    ...
    def do_it(self, seed, gif_path, prompt, all_subjects_data, args):   
        ...
        with torch.no_grad(): 
            ...
            for gif_subject_data in all_subjects_data:  
                ...
                placeholder_text = "a woman walking her baby in a SUBJECT0 " 
    ...
    with torch.no_grad(): 
        ...
        subjects = [
            [
               {
                    "subject": "pram", 
                    "normalized_azimuths": np.array([2.3 / 6] * NUM_SAMPLES),  
