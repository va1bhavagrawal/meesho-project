            ...
            for batch_idx in range(B): 
                for asset_idx, (idx1, idx2) in enumerate(encoder_hidden_states["attn_assignments"][batch_idx].items()):  
                    ...
                    if ("use_attn_centroid" in encoder_hidden_states["args"].keys() and encoder_hidden_states["args"]["use_attn_centroid"]): 
                        ...
                        assert False, "either use_attn_centroid should be True, or bboxes must be passed" 
                    ...
                    if self.loss_store is not None: 
                        ...
                        mean_j_bbox, mean_i_bbox = (bboxes[batch_idx][asset_idx][0] + bboxes[batch_idx][asset_idx][2]) / 2, (bboxes[batch_idx][asset_idx][1] + bboxes[batch_idx][asset_idx][3]) / 2  
                        mean_j_bbox, mean_i_bbox = INTERPOLATION_SIZE * mean_j_bbox, INTERPOLATION_SIZE * mean_i_bbox  
                        mean_j_bbox, mean_i_bbox = mean_j_bbox.item(), mean_i_bbox.item() 

                        loss = (mean_i_attn - mean_i_bbox) ** 2 + (mean_j_attn - mean_j_bbox) ** 2 
                    ...
                    box_resizing_factor = 0.75 + 0.75 * random.random()  
