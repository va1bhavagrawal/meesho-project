...
class Infer: 
    ...
    def generate_images_in_a_batch_and_save_them(self, batch, step_idx): 
        ...
        for batch_idx in range(len(all_assignments)): 
            ...
            for asset_idx in range(len(all_assignments[batch_idx])): 
                if asset_idx == 0: 
                    bboxes_example.append(torch.tensor([0.00, 0.58, 0.50, 1.00]))   
                elif asset_idx == 1: 
                    bboxes_example.append(torch.tensor([0.10, 0.30, 0.40, 0.55]))   
                elif asset_idx == 2: 
                    bboxes_example.append(torch.tensor([0.50, 0.30, 0.80, 0.55]))    
                elif asset_idx == 3: 
...
if __name__ == "__main__": 
    with torch.no_grad(): 
            ...
            [
               {
                ...
                {
                    "subject": "ship", 
                    "normalized_azimuths": np.array([0.65] * NUM_SAMPLES),  
                    "x": 0.7, 
                    "y": 0.7,  
                }, 
                {
        ...
        prompts = [
            ...
            # "a photo of PLACEHOLDER on a snowy street, festive Christmas lights, beautiful winter evening scene."
