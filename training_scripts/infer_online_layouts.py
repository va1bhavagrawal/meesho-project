    ...
    def generate_images_in_a_batch_and_save_them(self, batch, step_idx): 
        ...
        for batch_idx in range(len(all_assignments)): 
            ...
            for asset_idx in range(len(all_assignments[batch_idx])): 
                if asset_idx == 0: 
                    bboxes_example.append(torch.tensor([0.03, 0.65, 0.33, 0.95]))   
                elif asset_idx == 1: 
                    bboxes_example.append(torch.tensor([0.36, 0.65, 0.66, 0.95]))   
                elif asset_idx == 2: 
                    bboxes_example.append(torch.tensor([0.69, 0.65, 0.99, 0.95]))   
                elif asset_idx == 3: 
                    bboxes_example.append(torch.tensor([0.65, 0.45, 1.00, 0.70]))  
                elif asset_idx == 4: 
