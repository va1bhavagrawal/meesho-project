...
# WHICH_MODEL = "ablation_no_attn_regularization"
# WHICH_MODEL = "ablation_no_controlnet"
...
class Infer: 
    ...
    def generate_images_in_a_batch_and_save_them(self, batch, step_idx): 
        ...
        for batch_idx in range(len(all_assignments)): 
            ...
            for asset_idx in range(len(all_assignments[batch_idx])): 
                if asset_idx == 0: 
                    bboxes_example.append(torch.tensor([0.00, 0.50, 0.49, 1.00]))   
                elif asset_idx == 1: 
                    bboxes_example.append(torch.tensor([0.51, 0.50, 1.00, 1.00]))      
                elif asset_idx == 2: 
                    bboxes_example.append(torch.tensor([0.00, 0.35, 0.31, 0.67]))    
                elif asset_idx == 3: 
                    bboxes_example.append(torch.tensor([0.35, 0.35, 0.65, 0.67]))    
                elif asset_idx == 4: 
                    bboxes_example.append(torch.tensor([0.66, 0.35, 1.00, 0.67]))  
                elif asset_idx == 5: 
                    bboxes_example.append(torch.tensor([0.70, 0.47, 1.00, 0.695]))  
                else: 
                    assert False 
...
if __name__ == "__main__": 
    with torch.no_grad(): 
            ...
            [
               {
                ...
                {
                    "subject": "suv", 
                    "normalized_azimuths": np.array([0.5] * NUM_SAMPLES),  
                    "x": 0.7, 
                    "y": 0.7,  
                }, 
                {
        ...
        prompts = [
            ...
            "a photo of PLACEHOLDER on a road with trees on the side, high quality, best quality"
            # "a photo of PLACEHOLDER in a vast desert with towering sand dunes", 
            # "a photo of PLACEHOLDER in a racing stadium with a lot of people cheering", 
