...
def online_inference(args, pipeline, num_samples, special_tokens_ints_one, special_tokens_ints_two, special_encoder, special_encoder_two, tmp_dir, accelerator, wandb_log_data=None):  
	...
	for scene_idx, scene in enumerate(scenes_data): 
		...
		if accelerator.is_main_process: 
			images = [Image.open(osp.join(save_dir, f"{str(prompt_idx).zfill(3)}/img.jpg")) for prompt_idx in prompt_ids] 
...
def collate_fn(examples, tokenizer, with_prior_preservation=False):
	...
	for prompt_idx, prompt in enumerate(prompts): 
		...
		for subject_idx, subject_data in enumerate(example["subjects_data"]): 
			# assert prompt.find(f"<special_token_{subject_idx}>") != -1, f"{prompt = }" 
	...
	# 		prior_prompt = example["prior_prompt"] 
	# 		all_prompts.append(prior_prompt) 
	# 		prior_prompts_.append(prior_prompt)  
	# prompts = prompts_ 
	# prior_prompts = prior_prompts_  

	# input_ids = [example["prompt_ids"] for example in examples]
	pixel_values = [example["img_t"] for example in examples]

	# Concat class and instance examples for prior preservation.
	# We do this to avoid doing two forward passes.
	# if with_prior_preservation:
	# 	# input_ids += [example["class_prompt_ids"] for example in examples]
	# 	pixel_values += [example["prior_img_t"] for example in examples] 
	...
	# if with_prior_preservation: 
	# 	batch["prior_prompts"] = prior_prompts 
	# batch["all_prompt_ids"] = tokenizer(
	# 	all_prompts, 
	# 	truncation=True, 
	# 	padding="max_length", 
...
def main(args):
	args.output_dir = osp.join(args.output_dir, "_".join(args.run_name.split())) 
	...
	if args.optimizer.lower() == "adamw":
		...
		optimizers["backbone"] = optimizer_class(
			params_to_optimize,
	...
	if args.optimizer.lower() == "prodigy":
		raise NotImplementedError("prodigy optimizer is not yet supported") 
		try:
			import prodigyopt
		except ImportError:
	...
	torch.cuda.empty_cache() 
	train_datasets_stage1 = [
			RenderedImagesDataset(
				args=args,   
				imgs_dir=args.instance_data_dir_1subject, 
				include_special_tokens=True, 
			),  
			ControlNetImagesDataset(
				args=args, 
				imgs_dir=args.controlnet_data_dir_1subject,  
				ref_imgs_dir=args.instance_data_dir_1subject, 
				include_special_tokens=True, 
			)
	]
	print(f"{len(train_datasets_stage1[0]) = }")
	print(f"{len(train_datasets_stage1[1]) = }")
	...
	train_datasets_stage2 = [
			RenderedImagesDataset(
				args=args,   
				imgs_dir=args.instance_data_dir_2subjects, 
				include_special_tokens=True, 
			),  
			ControlNetImagesDataset(
				args=args, 
				imgs_dir=args.controlnet_data_dir_2subjects, 
				ref_imgs_dir=args.instance_data_dir_2subjects, 
				include_special_tokens=True, 
			)
	]
	...
	for name, optimizer in optimizers.items():  
		optimizers_[name] = accelerator.prepare(optimizer) 
	optimizers = optimizers_ 

	# We need to recalculate our total training steps as the size of the training dataloader may have changed.
	# STAGE 1  
	# overrode_max_train_steps = False 
	num_update_steps_per_epoch_stage1 = math.ceil(len(train_dataloader_stage1) / args.gradient_accumulation_steps)
	# if overrode_max_train_steps:
	# 	args.max_train_steps = args.num_train_epochs * num_update_steps_per_epoch_stage1
	# Afterwards we recalculate our number of training epochs
	args.num_train_epochs_stage1 = math.ceil(args.stage1_steps / num_update_steps_per_epoch_stage1)


	# overrode_max_train_steps = False 
	num_update_steps_per_epoch_stage2 = math.ceil(len(train_dataloader_stage2) / args.gradient_accumulation_steps)
	# if overrode_max_train_steps:
	# 	args.max_train_steps = args.num_train_epochs * num_update_steps_per_epoch_stage2 
	# Afterwards we recalculate our number of training epochs
	args.num_train_epochs_stage2 = math.ceil(args.stage2_steps / num_update_steps_per_epoch_stage2)

	save_steps = [500] 
	for save_step in range(1000, args.max_train_steps, 1250): 
	...
	if accelerator.is_main_process and args.wandb == True:
		...
		accelerator.init_trackers("iisc", config=tracker_config, init_kwargs=wandb_args) 
	...
	logger.info(f"  num examples in stage1 = {len(train_dataset_stage1)}")
	...
	if accelerator.is_main_process and (args.debug_dir is not None): 
	...
	if accelerator.is_main_process and args.run_name != "debug":  
		...
		assert not osp.exists(pkl_path)  
	...
	for epoch in range(first_epoch, args.num_train_epochs_stage1 + args.num_train_epochs_stage2):
		...
		if epoch < args.num_train_epochs_stage1: 
			train_dataloader = train_dataloader_stage1 
		...
		for step, batch in enumerate(train_dataloader):
			if args.debug_dir is not None: 
				...
				for key, value in batch.items(): 
					if type(value) == torch.Tensor: 
						continue 
					title += f"{key}: {value}\n" 
				for batch_idx in range(len(batch["pixel_values"])): 
					pixel_values = batch["pixel_values"][batch_idx].squeeze()  
					img = ((pixel_values * 0.5 + 0.5).cpu().numpy() * 255.0).astype(np.uint8) 
			...
			with accelerator.accumulate(unet):
				...
				add_time_ids = torch.cat(
					[
						compute_time_ids(original_size=(IMG_DIM, IMG_DIM), crops_coords_top_left=(0, 0))
				...
				if args.debug_dir is not None and global_step > 5: 
				# if True: 
					some_good_unet_param = False 
					for name, param in unet.named_parameters(): 
						if param.requires_grad: 
							# assert param.grad is not None and not torch.allclose(param.grad, torch.zeros_like(param.grad)), f"{name = }" 
							if param.grad is not None or not torch.allclose(param.grad, torch.zeros_like(param.grad)): 
								assert not torch.any(torch.isinf(param.grad)) and not torch.any(torch.isnan(param.grad)), f"{name = } is either inf or nan!"   
								some_good_unet_param = True 
					assert some_good_unet_param 
					# for name, param in text_encoder_one.named_parameters(): 
					# 	if param.requires_grad: 
					# 		assert param.grad is not None and not torch.allclose(param.grad, torch.zeros_like(param.grad)) 
					# for name, param in text_encoder_two.named_parameters(): 
					# 	if param.requires_grad: 
					# 		assert param.grad is not None and not torch.allclose(param.grad, torch.zeros_like(param.grad)) 
					some_good_param = False 
					for name, param in special_encoder.named_parameters(): 
						if param.requires_grad: 
