...
from custom_attention_processor import patch_custom_attention, get_attention_maps, show_image_relevance, SOFT_INFINITY, TEMPERATURE  

WHICH_LATENTS = "1"
WHICH_MODEL = "stage1_20000"     
...
if POSE_MASK_TYPE == "soft" or CLASS_MASK_TYPE == "soft": 
    KEYWORD = f"class_{CLASS_MASK_TYPE}__pose_{POSE_MASK_TYPE}__inf_{SOFT_INFINITY}__temp_{TEMPERATURE}"    
else: 
    KEYWORD = f"" 

# KEYWORD = KEYWORD + f"__mode_scaling_{MODE_SCALING}__final_comparison_bboxes3"
KEYWORD = f"0111__" + KEYWORD 
...
class Infer: 
    ...
    def generate_images_in_a_batch_and_save_them(self, batch, step_idx): 
        ...
        for batch_idx in range(len(all_assignments)): 
            ...
            for asset_idx in range(len(all_assignments[batch_idx])): 
                if asset_idx == 0: 
                    bboxes_example.append(torch.tensor([0.55, 0.55, 1.00, 1.00]))   
                elif asset_idx == 1: 
                    bboxes_example.append(torch.tensor([0.00, 0.20, 0.50, 0.50]))    
                elif asset_idx == 2: 
                    bboxes_example.append(torch.tensor([0.525, 0.65, 1.00, 1.00]))   
                elif asset_idx == 3: 
                    bboxes_example.append(torch.tensor([0.55, 0.25, 1.00, 0.60]))  
                elif asset_idx == 4: 
        ...
        for idx, image in enumerate(images): 
            ...
            #     cv2.rectangle(image, (bboxes[idx + B][2][:2].numpy() * img_dim).astype(np.int32), (bboxes[B + idx][2][2:].numpy() * img_dim).astype(np.int32), (255, 0, 0), 2) 
            # if MAX_SUBJECTS_PER_EXAMPLE > 3: 
            #     cv2.rectangle(image, (bboxes[idx + B][3][:2].numpy() * img_dim).astype(np.int32), (bboxes[B + idx][3][2:].numpy() * img_dim).astype(np.int32), (255, 255, 0), 2) 
            # if MAX_SUBJECTS_PER_EXAMPLE > 4: 
            #     cv2.rectangle(image, (bboxes[idx + B][4][:2].numpy() * img_dim).astype(np.int32), (bboxes[B + idx][4][2:].numpy() * img_dim).astype(np.int32), (255, 0, 255), 2) 
            # if MAX_SUBJECTS_PER_EXAMPLE > 5: 
            #     cv2.rectangle(image, (bboxes[idx + B][5][:2].numpy() * img_dim).astype(np.int32), (bboxes[B + idx][5][2:].numpy() * img_dim).astype(np.int32), (255, 128, 255), 2) 
    ...
    def do_it(self, seed, gif_path, prompt, all_subjects_data, args):   
        ...
        with torch.no_grad(): 
            ...
            for gif_subject_data in all_subjects_data:  
                ...
                # placeholder_text = "a black SUBJECT0 and a black SUBJECT1 and a black SUBJECT2 and a black SUBJECT3 and a black SUBJECT4 and a black SUBJECT5" 
                placeholder_text = placeholder_text.strip() 
                assert prompt.find("PLACEHOLDER") != -1 
                placeholder_text = "a girl riding SUBJECT0 on a bridge over a river, a SUBJECT1 is floating in the river below"
    ...
    with torch.no_grad(): 
        ...
        subjects = [
            [
               {
                    "subject": "bicycle", 
                    "normalized_azimuths": np.array([0.10] * NUM_SAMPLES),  
                ...
                {
                    "subject": "boat", 
                    "normalized_azimuths": np.array([0.0] * NUM_SAMPLES),  
                    "x": 0.7, 
                    "y": 0.7,  
                }, 
                {
                    "subject": "bicycle", 
                    "normalized_azimuths": np.array([0.90] * NUM_SAMPLES),  
                    "x": 0.7, 
                    "y": 0.7,  
                }, 
                {
                    "subject": "boat", 
                    "normalized_azimuths": np.array([0.675] * NUM_SAMPLES),  
                    "x": 0.7, 
                    "y": 0.7,  
                }, 
                {
                    "subject": "bicycle", 
                    "normalized_azimuths": np.array([0.375] * NUM_SAMPLES),  
                    "x": 0.7, 
                    "y": 0.7,  
                }, 
                {
        ...
        prompts = [
            ...
            "a photo of PLACEHOLDER on a bright sunny afternoon",  
