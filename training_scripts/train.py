...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="the file id of the object being trained",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=5,
        help=(
            "after how many ddp steps wandb logging should take place "
        ),
    )
    parser.add_argument(
        "--output_dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=25000,
        help="stage2 training steps to perform",
    )
    parser.add_argument(
        "--save_steps",
        type=int,
...
def main(args, controlnet_prompts):
    args.output_dir = osp.join(args.output_dir, args.subject + f"__{args.run_name}")  
    logging_dir = Path(args.output_dir, args.logging_dir)
    total_n_steps = args.stage1_steps + args.stage2_steps  
    ...
    lr_scheduler = get_scheduler(
        ...
        num_training_steps=total_n_steps * args.gradient_accumulation_steps,
    )
    ...
    progress_bar = tqdm(
        range(total_n_steps), disable=not accelerator.is_local_main_process
    )
    global_step = 0
    ddp_step = 0
    last_save = 0
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if global_step <= args.stage1_steps:
            progress_bar.set_description("stage 1:")
            encoder_hidden_states = text_encoder(batch["obj_ids"])[0]
        else:
            progress_bar.set_description("stage 2:")
            ...
            # setting requires_grad = False for the input embeddings 
            ...
            # storing the original input embeddings
            org_input_embeddings = torch.clone(text_encoder.module.get_input_embeddings().weight) 
            ...
            text_encoder.module.get_input_embeddings().weight = torch.nn.Parameter(org_input_embeddings)  
