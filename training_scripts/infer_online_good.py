...
WHICH_LATENTS = "1"
WHICH_MODEL = "bnha_at_output_corrected_resumed60000"     
...
if POSE_MASK_TYPE == "soft" or CLASS_MASK_TYPE == "soft": 
    KEYWORD = f"class_{CLASS_MASK_TYPE}__pose_{POSE_MASK_TYPE}__inf_{INFINITY}__temp_{TEMPERATURE}"    
else: 
    KEYWORD = f"" 

KEYWORD = KEYWORD + f"__mode_scaling_{MODE_SCALING}__bboxes5"
    ...
    def generate_images_in_a_batch_and_save_them(self, batch, step_idx): 
        ...
        for batch_idx in range(len(all_assignments)): 
            ...
            for asset_idx in range(len(all_assignments[batch_idx])): 
                if asset_idx == 0: 
                    bboxes_example.append(torch.tensor([0.05, 0.65, 0.45, 1.00]))   
                elif asset_idx == 1: 
                    bboxes_example.append(torch.tensor([0.55, 0.65, 0.95, 1.00]))   
                elif asset_idx == 2: 
                    bboxes_example.append(torch.tensor([0.15, 0.40, 0.45, 0.70]))   
                elif asset_idx == 3: 
                    bboxes_example.append(torch.tensor([0.55, 0.40, 0.85, 0.70]))  
                elif asset_idx == 4: 
                    bboxes_example.append(torch.tensor([0.30, 0.30, 0.70, 0.50]))  
                else: 
                    assert False 
        ...
        with torch.no_grad(): 
            ...
            for gif_subject_data in all_subjects_data:  
                ...
                for asset_idx, subject_data in enumerate(gif_subject_data): 
                    ...
                    # else: 
                    #     raise NotImplementedError("only 2 subjects supported for now!") 
                    track_ids.append(5 + asset_idx * 3) 
                    interesting_token_strs.append(f"SUBJECT{asset_idx}") 
                ...
                for sample_idx in range(n_samples): 
                    ...
                    for asset_idx, subject_data in enumerate(gif_subject_data): 
                        ...
                        # else: 
                        #     raise NotImplementedError("only 2 subjects supported for now!") 
                        attn_assignments[5 + asset_idx * 3] = 5 + asset_idx * 3 
                        special_embeddings[5 + asset_idx * 3] = merged_embs_video[sample_idx][asset_idx] 
...
if __name__ == "__main__": 
    with torch.no_grad(): 
        ...
        #     # "a photo of PLACEHOLDER in front of a serene waterfall with trees scattered around the region, and stones scattered in the region where the water is flowing",  
        #     # "a photo of PLACEHOLDER in a lush green forest with tall, green trees, stones are scattered on the ground in the distance, the ground is mushy and wet with small puddles of water",  
        #     # suggest more living room prompts 
        #     # please
        #     "a photo of PLACEHOLDER in a modern living room setting, surrounded by glass windows and neatly painted walls, the room is well lit with sunlight",   
        #     # "a photo of PLACEHOLDER in a field of dandelions, with the sun shining brightly, there are snowy mountain ranges in the distance",   
        # ]
        # for prompt in prompts: 
