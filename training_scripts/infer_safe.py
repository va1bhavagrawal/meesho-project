...
WHICH_MODEL = "__randomxy_freezeapp" 
WHICH_STEP = 110000 
...
class Infer: 
    def __init__(self, gif_name, accelerator, unet, scheduler, vae, text_encoder, tokenizer, mlp, merger, bnha_embeds=None, bs=6):   
    ...
    def do_it(self, prompt, subjects, n_samples, pose_type, appearance_type, include_class_in_prompt=True):    
        with torch.no_grad(): 
            ...
            elevation = 0 
            radius = 2.0 
            fixed_azimuth = normalized_azimuths[np.random.randint(0, len(normalized_azimuths))] 
            for azimuth_idx, azimuth in enumerate(normalized_azimuths):  
                # azimuth = fixed_azimuth 
                azimuth = 3.5 / (2 * math.pi)  
                curr_seed += 1 
                set_seed(curr_seed) 
                ...
                for subject_idx, subject in enumerate(subjects):  
                    ...
                    if pose_type == "0" and appearance_type == "zero": 
                        subject_prompt = subject_prompt.replace("bnha", "") 
...
if __name__ == "__main__": 
    with torch.no_grad(): 
        ...
        for inference_type in inference_types: 
            ...
            infer = Infer(
                # f"{prompt_}.gif", 
                f"dark_singleview{pose_type}_{appearance_type}.gif",  
                accelerator, 
                pipeline.unet, 
                pipeline.scheduler, 
                pipeline.vae, 
            ...
            prompt = "a photo of a SUBJECT on a bustling city street surrounded by towering skyscrapers and neon lights" 
            infer.gif_name = f"neon_singleview{pose_type}_{appearance_type}.gif" 
            n_samples = 6  
            infer.do_it(
                prompt,  
                subjects, 
            ...
            prompt = "a photo of a SUBJECT on a beach with swaying palm trees"   
            infer.gif_name = f"beach_singleview_{pose_type}_{appearance_type}.gif" 
            n_samples = 6  
            infer.do_it(
                prompt,  
                subjects, 
