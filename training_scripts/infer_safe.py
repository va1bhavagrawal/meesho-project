...
WHICH_MODEL = "__skippose_posedropout" 
WHICH_STEP = 50000  
...
class Infer: 
    def __init__(self, gif_name, accelerator, unet, scheduler, vae, text_encoder, tokenizer, mlp, merger, bnha_embeds=None, bs=2):   
        ...
        self.bs = bs 
        self.vis_dir = osp.join(f"inference_results", f"{WHICH_MODEL}_{WHICH_STEP}", "tmp") 
    ...
    def do_it(self, prompt, subjects, n_samples, pose_type, appearance_type, include_class_in_prompt=False):    
        with torch.no_grad(): 
            ...
            for azimuth_idx, azimuth in enumerate(normalized_azimuths):  
                ...
                # print(f"the pose type is {pose_type}") 
                if pose_type == "zero":  
                    print(f"pose type has been set to zero!")
                ...
                #     print(f"appearance type has been set to zero!") 
                #     bnha_embs = torch.zeros_like(bnha_embs) 
...
if __name__ == "__main__": 
    with torch.no_grad(): 
        ...
        lora_path = ckpt_path.replace(basename_ckpt, f"lora_weight_100000.safetensors") 
        ...
        for inference_type in inference_types: 
            ...
            prompt = "a photo of a SUBJECT on a beach"  
            prompt_ = "_".join(prompt.split()) 
            pose_type, appearance_type = inference_type 
            # infer = Infer(
            #     # f"{prompt_}.gif", 
            #     f"beach_{pose_type}_{appearance_type}.gif",  
            #     accelerator, 
            #     pipeline.unet, 
            #     pipeline.scheduler, 
            #     pipeline.vae, 
            ...
            #     # bnha_embeds,  
            # )
            infer.gif_name = f"beach_{pose_type}_{appearance_type}.gif"  
            ...
            # infer = Infer(
            #     # f"{prompt_}.gif", 
            #     f"dark_{pose_type}_{appearance_type}.gif",  
            #     accelerator, 
            #     pipeline.unet, 
            #     pipeline.scheduler, 
            #     pipeline.vae, 
            ...
            #     # bnha_embeds,  
            # )
            infer.gif_name = f"dark_{pose_type}_{appearance_type}.gif"  
            ...
            prompt = "a photo of a SUBJECT in a field of blooming sunflowers with snowy mountain ranges in the distance"   
            prompt_ = "_".join(prompt.split()) 
            pose_type, appearance_type = inference_type 
            # infer = Infer(
            #     # f"{prompt_}.gif", 
            #     f"sunflowers_{pose_type}_{appearance_type}.gif",  
            #     accelerator, 
            #     pipeline.unet, 
            #     pipeline.scheduler, 
            #     pipeline.vae, 
