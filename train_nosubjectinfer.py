    ...
    with torch.no_grad(): 
        ...
        for azimuth in range(prompts_dataset.num_samples): 
            if azimuth % accelerator.num_processes == accelerator.process_index: 
                ...
                # print(f"all the parameters of merger are zero during inference! (assertion passed!)") 
                ...
                for i, merged_emb in enumerate(merged_embs):  
                    ...
                    # assert torch.allclose(merged_embs[i], input_embeddings[TOKEN2ID["motorbike"]])  

                # accelerator.unwrap_model(text_encoder).get_input_embeddings().weight[TOKEN2ID["bnha"]] = mlp(sincos.unsqueeze(0)) 
                    # assert torch.allclose(accelerator.unwrap_model(text_encoder).get_input_embeddings().weight[TOKEN2ID["bnha"]], merged_embs[i])  
                    # assert torch.allclose(accelerator.unwrap_model(text_encoder).get_input_embeddings().weight[TOKEN2ID["bnha"]], accelerator.unwrap_model(text_encoder).get_input_embeddings().weight[TOKEN2ID["motorbike"]]) 
        ...
        for batch in tqdm(encoder_states_dataloader, disable = not accelerator.is_main_process):  
            ...
            for idx, image in zip(ids, images):  
                ...
                os.makedirs(save_path_prompt, exist_ok=True) 
                image = Image.fromarray(image) 
                img_save_path = osp.join(save_path_prompt, f"{str(int(azimuth.item())).zfill(3)}.jpg")
                # make sure that you are not overwriting 
                # assert not osp.exists(img_save_path) 
        ...
        # setting common seed so that the latents are the same for all the types of generation 
        common_seed = get_common_seed() 
        set_seed(common_seed)  
...
def main(args): 
    ...
    #     eps=args.adam_epsilon,
    # )
    optimizer_mlp = torch.optim.SGD(continuous_word_model.parameters(), lr=args.learning_rate_mlp) 
    ...
    # for p in merger.parameters(): 
    #     assert torch.allclose(p, torch.zeros_like(p)) 
    # print(f"merger is zero in the main function just after initialization!") 
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if DEBUG: 
            with torch.no_grad(): 
                ...
                # print(f"\n\n\n\n\n\n\n\n")
                print(f"\n")
                print(f"{len(check_mlp_params) = }")
                for n, p in continuous_word_model.named_parameters(): 
                    # print(f"at least there is a parameter :: ", end="") 
                    # print(f"{p.grad = }")
                    print(f"{n = }, {p.shape = }")
                    if p.grad is not None: 
                        print(f", {torch.sum(p.grad).item() = }")
                    if p.grad is None: 
                        continue 
                    print(f"ATLEAST NOT NONE!") 
                    if not torch.allclose(torch.zeros_like(p.grad), p.grad):  
                        print(f"SOMETHING IN THE MLP MAKES SENSE!") 
                # print(f"\n\n\n\n\n\n\n\n")
                print(f"\n")
                # assert not ((len(check_mlp_params) < len(list(continuous_word_model.parameters())) ^ (global_step > args.stage1_steps))), f"{check_mlp_params[0] = }"  
        ...
        if DEBUG: 
            with torch.no_grad(): 
                ...
                mlp_before = copy.deepcopy([torch.clone(p) for p in continuous_word_model.parameters()])  
        ...
        if DEBUG: 
            ...
            with torch.no_grad(): 
                ...
                mlp_after = copy.deepcopy([torch.clone(p) for p in continuous_word_model.parameters()])  
                if global_step > args.stage1_steps: 
                    change = False 
                    for p1, p2 in zip(mlp_before, mlp_after): 
